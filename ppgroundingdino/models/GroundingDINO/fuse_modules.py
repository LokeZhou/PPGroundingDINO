...
class BiMultiHeadAttention(nn.Layer):
    ...
    def forward(self, v, l, attention_mask_v=None, attention_mask_l=None):
        ...
        if attention_mask_v is not None:
            attention_mask_v = (
                attention_mask_v[:, None, None, :].tile([1, self.num_heads, 1, 1]).cast(paddle.float32).flatten(0, 1)
            )
        ...
        if attention_mask_l is not None:
            attention_mask_l = (
                attention_mask_l[:, None, None, :].tile([1, self.num_heads, 1, 1]).cast(paddle.float32).flatten(0, 1)
            )
