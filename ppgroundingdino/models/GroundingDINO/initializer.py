...
def _no_grad_trunc_normal_(tensor, a, b):
    """ trunc_normal for standard normal dirstribution
    """
    with paddle.no_grad():
        l = math.erf(a / math.sqrt(2.))
        u = math.erf(b / math.sqrt(2.))
        tmp = paddle.uniform(shape=tensor.shape, dtype=tensor.dtype, min=l, max=u)
        tmp = paddle.erfinv(tmp) * math.sqrt(2.)
        tmp = paddle.clip(tmp, a, b)
        tensor.set_value(tmp)
    
    return tensor


def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    return _no_grad_trunc_normal_(tensor, a, b) * std + mean
