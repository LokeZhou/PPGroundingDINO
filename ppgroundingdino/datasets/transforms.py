...
from ppgroundingdino.util.box_ops import box_xyxy_to_cxcywh
from ppgroundingdino.util.misc import interpolate
...
def crop(image, target, region):
    ...
    if "boxes" in target:
        boxes = target["boxes"]
        max_size = paddle.to_tensor([w, h], dtype=paddle.float32)
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        image = T.normalize(image, mean=self.mean, std=self.std)
        if target is None:
            return image, None
        target = target.copy()
        h, w = image.shape[-2:]
        if "boxes" in target:
            ...
            boxes = boxes / paddle.to_tensor([w, h, w, h], dtype=paddle.float32)
            target["boxes"] = boxes
        return image, target
